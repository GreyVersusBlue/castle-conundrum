...
three clips the kit already ships, and no new asset. Decisions #607 to #610,
**written as #604 to #606 and renumbered on the merge**: rank 1's fourth
body landed on `main` while this was being built and took #603 to #606,
which is the collision that row's own entry above predicted. A decision
number is not a lane, two sessions can reach for the same one, and the
session that merges second moves. #610 came with the merge as well: rank 1's
paperwork was left for whoever landed `BACKLOG.md` and `SPECS.md`, and that is
this branch.
...
see #609's second half.

- **A routine is a ring inside a watch, not a timetable** (#607).
...
- **A label never out-ranks something to press E at** (#608).
...
- **`garden` is a room id nothing in this castle resolves to, and `npm test`
  was already red on Windows** (#609). Two things found on the way and
...
- **A closed rank is retired, not reused, and rank 1 came out here** (#610).
