...
hall itself, directly. Decisions #636 to #638.

- **`roof.glb` is the whole cross-section, ridge and both slopes, in one
  piece — not a single-pitch wedge that needs a mate** (#636). `partsOf`
...
- **Bay 1's west edge is narrower than the other six — 3.7 m, not 4 — because
  ...
  never do** (#637). `test/layout.mjs` check 14 failed once, cleanly:
...
- **The floor, measured at Vespers with all six of the hall's cast in place,
  reads well clear of the line SPECS.md drew** (#638). The run that would
