...
(#624 to #630 on 2026-09-17, #708 to #715 on 2026-09-19, #725 to #732 on
...
the reload at Sext, the riddle, the third ring and the cook's walk (#725).
Six fixes to `test/play-castle.mjs` then carried the second run to the
accusation (#726 to #731), where the sentry and the porter went unread and
the ending came out wrong (#732). What is left is `src/interaction.js`'s
...
- **Nothing in `src/`.** The run is the deliverable. `test/play-castle.mjs` is
  ...
  2026-09-21 (#726 to #731): `shutPresent()` closes the Present list and the
...
- ~~The `snap('twelve-at-vespers')` beat~~ was routing straight across the
  inner ward into a wall; **fixed 2026-09-21** (#731), it now walks to the
...
- ~~`shots/play/` contains the numbered set, `twelve-at-vespers.png` among
  ...
  routing fix (#731) got the walked, numbered set past it on 2026-09-21.
...
- **Rank 1, the walker on the stair, shipped 2026-09-19 and is now confirmed
  on a GPU** (#716 to #720, #725, #727). `hike` no longer drives the player up
