...
**Rank 2f. Size 1. Model Opus 5. Where: any machine with KTX-Software's
`ktx` on PATH, because `npm run assets:encode` refuses to start without it.
No Blender and no GPU are needed. Gate: none. Lane B.** Devon, 2026-09-25:
"We now have all of these props in assets/props. Can we go in and place
these?" Commit `51735fa` added 85 `.glb` files and a README, made by his own
Blender script outside this repo. Decided in `HISTORY.md` as #830 to #834
against `51735fa`; this section is the `builder` job those decisions leave,
one increment, class S. Every row below was spliced into a prototype and run
against the suites before it was written here (#833's entry says what held).

**What the files are, measured.** 85 files, 108 primitives, 4.2 MB. Each has
one material, `KHR_materials_specular`, metallic 0, and the same embedded
128 x 128 RGBA PNG atlas, NEAREST: byte-identical in all 85 (sha256
`e58ea65f8c61...`), with 1821 colours and none in `tools/pixel/`'s palette
union. The two cobwebs and `net-rack` use an alpha mask and are
double-sided. Front faces +Z, the base is at y 0, and the origin is at the
base centre, so a row's `tile` is its box centre. Wall-hung pieces have
their back at -Z: at rotationY 0 a piece hangs on a north wall (the smaller
z), at 180 a south wall, at 90 a west wall, at -90 an east wall. A cobweb's
corner is at local (-x, -z), so at 0 it fills a north-west corner, at -90 a
north-east, at 180 a south-east and at 90 a south-west. The draw count of
a file is its primitive count. Twenty files have two nodes or more:
`ballista`, `bread-oven-door`, `cell-door-barred`, `chest-iron-banded`,
`covered-well`, `gaming-table`, `garderobe-door`, `hearth-crane-cauldron`,
`quay-crane`, `secret-bookcase`, `secret-turning-sconce`, `spit-rotisserie`,
`strongbox`, `toll-house-sign`, `travel-chest`, `trestle-table-set`,
`wall-torch-sconce` and `writing-desk-locked` are 2 each, `bell-and-rope`
is 3 and `body-scene-set` is 4.

### Scope (class S)

- **`src/castle-plan.js`** (#832):
  - `export function propPath(base, model)`: `model` if it starts
    `assets/`, else `base + model`. This is `heldPropPath`'s rule;
    `src/populace.js` keeps its own function.
  - The `interiorProps` loop: `const model = propPath(pBase, p.model)`, read
    by `boundsOf` and written as the piece's `model`. A row whose `model`
    starts `assets/` with no `id` throws `interiorProps: <model> has no id
    (#812)`.
  - `base`, read the way `courtyard.placements` reads it: `const base =
    p.base || 0, lvl = levelUnder(base)`; `lift = Math.max(base,
    surfaceHeightUnder(stack.filter((s) => levelUnder(s.min.y) === lvl),
    flat.box)) + (p.yOffset || 0)`; the piece's and the surface's `level` is
    `lvl`, not 0. With no `base`, every existing row is placed to the
    millimetre as before, which is what `plan-vs-scene.mjs` confirms.
- **`propPath` at the other six sites**: `src/castle-builder.js`
  `modelPaths`; `tools/encode-assets.mjs` line 250; `test/budget.mjs` line
  524; `test/assets.mjs` lines 131, 474 and 539. The id at the budget and
  check 4 sites stays `p.id || p.model.split('/')[0]`.
- **`tools/encode-assets.mjs`** (#831): in `encodeGLTF`'s texture loop,
  `continue` past a texture whose image starts with the PNG signature and
  whose IHDR width and height (big-endian at bytes 16 and 20) are both 128
  or under. Create `KHRTextureBasisu` only if at least one texture in the
  file was encoded. The header's size paragraph says this is where the
  exemption lives now.
- **`tools/place.mjs`**: `'base'` in `PLACEABLE.interiorProps`, after
  `'yOffset'`.
- **`src/edit-mode.js`**: an `interiorProps` row whose model starts
  `assets/` takes its `id` from the id field and is refused without one, as
  a `builtProps` row is. A row placed above the ground carries `base:
  at.base`. Without the first, the editor writes a row that stops the page
  loading.
- **`test/layout.mjs` check 1d**: the slabs it holds are joined by every
  `interiorProps` row with a `base` and no `noCollide`, found by id in the
  config and measured the same way: something under its base within 0.05 m.
- **`test/assets.mjs`**: check 4's sweep walks `assets/props` beside the
  other three. **New check 9, "Devon's props"** (#831), over every `.glb`
  under `assets/props/`: carries `EXT_meshopt_compression`, has one
  material and one image, the image is `image/png` with an IHDR of at most
  `PIXEL_PX` on both sides, and `extensionsUsed` has no
  `KHR_texture_basisu`. Check 8 is rank 1's number and stays free.
- **`data/scene-config.json`**: the 70 rows below, appended to
  `interiorProps` in the order given, each through `tools/place.mjs`'s
  `insertRow` in the file's own line ending (#584, #632). Asset and row
  land in one commit (#390).
- **`assets/props/`**: after the rows are in, `npm run assets:encode`. It
  meshopt-encodes the 69 placed files and, with the size skip, leaves each
  atlas a PNG. A second run prints "nothing to do" for every one of them.
  `git rm` the 16 files listed under "Not placed". `git mv
  assets/props/README.md tools/props/README.md`, and change its sentence
  "Nothing here is meshopt-encoded yet" to say the placed files are
  encoded and the 16 are in `51735fa`.
- **`tools/props/`** (#830): copies of `build.py`, `kit.py`, `atlas.py`,
  `registry.py`, `props_evidence.py`, `props_rooms.py`, `props_large.py`
  and `manifest.json` from `Claude Files/Blender Projects/Castle/_source/`,
  byte for byte. Nothing else from that folder: no `.blend`, no `.blend1`,
  no `previews/`, no `__pycache__/`, no atlas PNG. `build.py` still names
  `assets\props` as an output. The README says so and says a re-export
  writes raw files, which check 5 and check 9 then fail until
  `assets:encode` runs again.
- **Lead's one-liners**: `README.md`'s credits say `assets/props/` is
  Devon's own work, made with his script in Blender; `CLAUDE.md`'s
  exemption sentence names the props' atlas beside the Kenney kit and the
  fifteen pixel textures.
- **Untouched**: `data/mystery.json`, `data/npcs.json`,
  `data/populace.json`, `data/quests/`, `src/save.js` (no version bump),
  `src/interaction.js`, `src/scene-setup.js` (no light), `test/mystery.mjs`,
  `test/plan-vs-scene.mjs`, every ceiling in `test/budget.mjs`.

### The rows (#833)

By room, with the draws each adds. `base` is a floor:
4 is a first floor, 8 a top room, 12 a roof.

| Room | Rows | Draws | Ward |
| --- | --- | ---: | --- |
| `kitchen` | hearth crane, firewood, spit, butcher block, herbs (hung), sacks, bread oven | 10 | outer |
| `great-hall` | fireplace, the lord's high chair on the dais, goblet on `WoodenTable_01`, banner A | 4 | outer |
| `dormitory` (base 4) | weapon rack, armour stand, gaming table, watch-bill board, muddy boot | 6 | outer |
| `clerk-office`, `larder` | wax seal on the roped crate; vial on the crate, apple barrel | 3 | outer |
| outer ward, `prison-tower-roof` (base 12) | covered well; ballista | 4 | outer |
| `kings-hall` | trestle set and boar, banners B and C, tapestry A, two torch sconces, four crest shields, chandelier, large rug, window, cobweb, brazier | 19 | inner |
| `royal-apartments` (base 4) | four-poster bed, basin, travel chest, garderobe, iron chest, tapestry B, medium rug, lantern, cobweb | 12 | inner |
| `steward-chamber` | bookshelf, lectern, book press, locked desk with ledger, inkwell and candlestick, strongbox, tapestry C, small rug, manuscripts | 13 | inner |
| `porter-lodge`, `kings-tower-1` (base 4) | keys on the barrels; secret bookcase, turning sconce | 5 | inner |
| `chapel`, `chapel-tower-2` (base 8) | censer; altar, font, pew | 4 | inner |
| inner ward, east of the Steward's chamber | raised bed, bucket, scarecrow, beehive, sundial, shears | 6 | inner |
| **total** | **70 rows, 69 files** | **86** | outer 27, inner 59 |

These match `test/budget.mjs`'s own count on the prototype: no row's box
reaches across the cross-wall, so each draw lands in one ward. The builder
records the lines `budget.mjs` prints, not this table.

The rows, in order, for `insertRow`:

```json
{"id":"kitchen-hearth-crane","model":"assets/props/hearth-crane-cauldron.glb","tile":[-5.8,-1.6925],"rotationY":180}
{"id":"kitchen-firewood","model":"assets/props/firewood-stack.glb","tile":[-6.15,-1.7225]}
{"id":"kitchen-spit","model":"assets/props/spit-rotisserie.glb","tile":[-4.25,-1.66625],"rotationY":180}
{"id":"kitchen-butcher-block","model":"assets/props/butcher-block.glb","tile":[-5.625,-2.75],"rotationY":20}
{"id":"kitchen-herbs","model":"assets/props/herb-bundles.glb","tile":[-5.25,-2.25],"yOffset":3.27,"noCollide":true}
{"id":"kitchen-sacks","model":"assets/props/sack-pile.glb","tile":[-3.775,-2.65],"rotationY":-90}
{"id":"kitchen-bread-oven","model":"assets/props/bread-oven-door.glb","tile":[-6.2975,-2.575],"rotationY":90}
{"id":"hall-fireplace","model":"assets/props/stone-fireplace.glb","tile":[-6.75,1.795]}
{"id":"hall-high-chair","model":"assets/props/lords-high-chair.glb","tile":[-2.25,2.5],"rotationY":-90,"yOffset":0.32}
{"id":"hall-goblet","model":"assets/props/goblet-poisoned.glb","tile":[-4.91,2.625],"noCollide":true}
{"id":"hall-banner-a","model":"assets/props/heraldic-banner-a.glb","tile":[-1.63625,2.85],"rotationY":-90,"yOffset":1.5,"noCollide":true}
{"id":"dormitory-weapon-rack","model":"assets/props/weapon-rack.glb","tile":[-3.665,-2.75],"rotationY":-90,"base":4}
{"id":"dormitory-armour-stand","model":"assets/props/armour-stand.glb","tile":[-3.8,-2],"rotationY":-90,"base":4}
{"id":"dormitory-gaming-table","model":"assets/props/gaming-table.glb","tile":[-5.375,-2.75],"base":4}
{"id":"dormitory-watch-bill","model":"assets/props/watch-bill-board.glb","tile":[-6.36125,-2.875],"rotationY":90,"base":4,"yOffset":1.2,"noCollide":true}
{"id":"dormitory-boot","model":"assets/props/muddy-boot.glb","tile":[-4.875,-1.75],"rotationY":30,"base":4}
{"id":"clerk-seal","model":"assets/props/wax-seal-stamp.glb","tile":[-8.1,-1.85],"yOffset":0.45,"noCollide":true}
{"id":"larder-vial","model":"assets/props/vial-residue.glb","tile":[-5.4,-4.1],"yOffset":0.45,"noCollide":true}
{"id":"larder-apples","model":"assets/props/apple-barrel.glb","tile":[-5.4,-4.375]}
{"id":"outer-well","model":"assets/props/covered-well.glb","tile":[-4.25,-0.5]}
{"id":"prison-roof-ballista","model":"assets/props/ballista.glb","tile":[-5.05,3.75],"rotationY":90,"base":12}
{"id":"kings-hall-trestle","model":"assets/props/trestle-table-set.glb","tile":[3.375,-3.1]}
{"id":"kings-hall-boar","model":"assets/props/roasted-boar-platter.glb","tile":[3.3,-3.1],"yOffset":-0.22,"noCollide":true}
{"id":"kings-hall-banner-b","model":"assets/props/heraldic-banner-b.glb","tile":[1.375,-3.48875],"yOffset":1.5,"noCollide":true}
{"id":"kings-hall-sconce-west","model":"assets/props/wall-torch-sconce.glb","tile":[2,-3.47125],"yOffset":1.6,"noCollide":true}
{"id":"kings-hall-tapestry","model":"assets/props/tapestry-a.glb","tile":[3.375,-3.49],"yOffset":1.2,"noCollide":true}
{"id":"kings-hall-sconce-east","model":"assets/props/wall-torch-sconce.glb","tile":[4.25,-3.47125],"yOffset":1.6,"noCollide":true}
{"id":"kings-hall-banner-c","model":"assets/props/heraldic-banner-c.glb","tile":[4.75,-3.48875],"yOffset":1.5,"noCollide":true}
{"id":"kings-hall-shield-a","model":"assets/props/crest-shield-a.glb","tile":[1.125,-1.63625],"rotationY":180,"yOffset":1.9,"noCollide":true}
{"id":"kings-hall-shield-b","model":"assets/props/crest-shield-b.glb","tile":[2,-1.63625],"rotationY":180,"yOffset":1.9,"noCollide":true}
{"id":"kings-hall-shield-c","model":"assets/props/crest-shield-c.glb","tile":[3.75,-1.63625],"rotationY":180,"yOffset":1.9,"noCollide":true}
{"id":"kings-hall-shield-d","model":"assets/props/crest-shield-d.glb","tile":[4.625,-1.63625],"rotationY":180,"yOffset":1.9,"noCollide":true}
{"id":"kings-hall-chandelier","model":"assets/props/chandelier-ring.glb","tile":[3.375,-2.5],"yOffset":2.45,"noCollide":true}
{"id":"kings-hall-rug","model":"assets/props/rug-large.glb","tile":[2.125,-2.5],"noCollide":true}
{"id":"kings-hall-cobweb","model":"assets/props/cobweb-corner-a.glb","tile":[5.4225,-1.69125],"rotationY":180,"yOffset":3.18,"noCollide":true}
{"id":"royal-bed","model":"assets/props/four-poster-bed.glb","tile":[2.375,-3.20875],"base":4}
{"id":"royal-basin","model":"assets/props/wash-basin-stand.glb","tile":[2.825,-3.435],"base":4}
{"id":"royal-travel-chest","model":"assets/props/travel-chest.glb","tile":[2.375,-2.8],"base":4}
{"id":"royal-garderobe","model":"assets/props/garderobe-door.glb","tile":[5.34625,-2.25],"rotationY":-90,"base":4}
{"id":"royal-chest","model":"assets/props/chest-iron-banded.glb","tile":[3.75,-3.415],"base":4}
{"id":"royal-tapestry","model":"assets/props/tapestry-b.glb","tile":[2.375,-1.635],"rotationY":180,"base":4,"yOffset":1,"noCollide":true}
{"id":"royal-rug","model":"assets/props/rug-medium.glb","tile":[2.375,-2.475],"base":4,"noCollide":true}
{"id":"royal-lantern","model":"assets/props/hanging-lantern.glb","tile":[3.25,-2.5],"base":4,"yOffset":2.6,"noCollide":true}
{"id":"royal-cobweb","model":"assets/props/cobweb-corner-b.glb","tile":[5.45375,-1.66475],"rotationY":180,"base":4,"yOffset":3.43,"noCollide":true}
{"id":"steward-bookshelf","model":"assets/props/bookshelf-wall.glb","tile":[2.31875,2.625],"rotationY":-90}
{"id":"steward-lectern","model":"assets/props/lectern-slanted.glb","tile":[0.5725,2.5],"rotationY":90}
{"id":"steward-book-press","model":"assets/props/book-press.glb","tile":[1.875,3.4475],"rotationY":180}
{"id":"steward-desk","model":"assets/props/writing-desk-locked.glb","tile":[1.25,3.41625],"rotationY":180}
{"id":"steward-ledger","model":"assets/props/ledger.glb","tile":[1.2625,3.425],"rotationY":180,"yOffset":-0.17,"noCollide":true}
{"id":"steward-inkwell","model":"assets/props/inkwell-quill-cup.glb","tile":[1.17,3.4375],"rotationY":180,"yOffset":-0.17,"noCollide":true}
{"id":"steward-candlestick","model":"assets/props/candlestick-brass-bent.glb","tile":[1.335,3.4375],"yOffset":-0.17,"noCollide":true}
{"id":"steward-strongbox","model":"assets/props/strongbox.glb","tile":[2.3,1.75],"rotationY":-90}
{"id":"steward-tapestry","model":"assets/props/tapestry-c.glb","tile":[0.51,2.125],"rotationY":90,"yOffset":1,"noCollide":true}
{"id":"steward-rug","model":"assets/props/rug-small.glb","tile":[1.5,2.7],"noCollide":true}
{"id":"steward-manuscripts","model":"assets/props/manuscript-stack.glb","tile":[2.3,2.1],"rotationY":-90}
{"id":"porter-keys","model":"assets/props/key-ring.glb","tile":[0.4,-3.65],"yOffset":0.79,"noCollide":true}
{"id":"kings-tower-bookcase","model":"assets/props/secret-bookcase.glb","tile":[6,-4.5],"base":4}
{"id":"kings-tower-sconce","model":"assets/props/secret-turning-sconce.glb","tile":[6.25,-4.55],"base":4,"yOffset":1.5,"noCollide":true}
{"id":"kings-hall-window","model":"assets/props/stained-glass-window.glb","tile":[2.5,-1.6725],"rotationY":180,"yOffset":1.2,"noCollide":true}
{"id":"chapel-censer","model":"assets/props/censer-hanging.glb","tile":[5.775,3.9],"yOffset":2.4,"noCollide":true}
{"id":"chapel-loft-altar","model":"assets/props/altar.glb","tile":[6.075,3.51625],"base":8}
{"id":"chapel-loft-font","model":"assets/props/stone-font.glb","tile":[6.325,3.775],"base":8}
{"id":"chapel-loft-pew","model":"assets/props/pew-section.glb","tile":[6.0125,3.805],"rotationY":180,"base":8}
{"id":"garden-raised-bed","model":"assets/props/raised-bed.glb","tile":[3.5,2.875]}
{"id":"garden-bucket","model":"assets/props/well-bucket.glb","tile":[3.825,2.9]}
{"id":"garden-scarecrow","model":"assets/props/scarecrow.glb","tile":[4.2,2.8]}
{"id":"garden-beehive","model":"assets/props/beehive-skep.glb","tile":[5.44375,2],"rotationY":-90}
{"id":"garden-sundial","model":"assets/props/sundial.glb","tile":[3.125,2]}
{"id":"garden-shears","model":"assets/props/garden-shears.glb","tile":[3.5,2.675],"rotationY":15,"noCollide":true}
{"id":"kings-hall-brazier","model":"assets/props/brazier.glb","tile":[4.375,-2.8]}
```

What stacks: `hall-goblet` on `WoodenTable_01` (its top is its box, 0.55);
`kings-hall-boar` on the trestle, and the three steward pieces on the
desk, by the negative `yOffset` #832 explains. `clerk-seal`, `larder-vial`
and `porter-keys` sit on kit placements, which are not in the stack, so
their `yOffset` is the kit piece's top typed once (0.45, 0.45, 0.79). That
is the same kind of number check 1d holds for slabs, and it is held here by
the GPU look, not a rail. `hall-high-chair`'s 0.32 is the dais's
`floor-steps` height, for the same reason.

### Not placed, and where each goes (#833)

All 16 are deleted in the placing commit (check 4) and restored from
`51735fa` by whichever row places them.

| File | Why not here | Whose |
| --- | --- | --- |
| `body-scene-set` | a body on the floor on all three days, including the walking day, when Hywel is alive | nobody's without a day overlay in `mystery.json` |
| `dagger-crested` | a loose blade in a mystery whose knife clues are `knife-missing` and `pouch-empty` | nobody's |
| `torn-letter` | the shape of `summons-note`, a clue on paper | nobody's |
| `signet-ring` | 2.8 cm; `interiorProps` has no scale | a row that adds one |
...
`npm test` fifteen of fifteen. Each line has the break that must turn it
red from green (#34), after the rows, the encode and the deletions.

1. **Check 9, new** (`test/assets.mjs`). **The break the builder quotes**:
   take the size skip out of `encodeGLTF` and run `npm run
   assets:encode`. Every placed atlas becomes KTX2. Expected: "assets/props/
   altar.glb's image is image/ktx2, not a PNG of 128 px or under (#831)",
   and one line for each other file. Restore with `git checkout --
   assets/props tools/encode-assets.mjs`.
2. **Check 4's sweep over `assets/props`.** Break: delete the `royal-bed`
   row. Expected: "nothing references assets/props/four-poster-bed.glb".
3. **Check 5, unchanged**, now finding the props through `propPath`. Break:
   `git checkout 51735fa -- assets/props/altar.glb`. Expected: "has no
   EXT_meshopt_compression".
4. **The id throw** (#812), which surfaces in every Node suite. Break: delete
   `royal-bed`'s `id`. Expected: `makePlan` throws "interiorProps:
   assets/props/four-poster-bed.glb has no id".
5. **Check 1d over `base`.** Break: `royal-bed`'s `base` 4.3. Expected:
   "royal-bed has its base at y 4.30 and nothing under it within 0.05 m".
6. **Held with no new line**: check 1 (break: `kitchen-hearth-crane` 0.5 m
   south, into the kitchen's south wall), 1b, 3, 6 (a prop on a flight's
   landing cuts a tower off from its own stair, which is how the prototype's
   first brazier was found), 11; `validateMystery` and `validatePopulace`
   with no station moved (#814); the allow-list rail in `test/tools.mjs`,
   which `base` passes only because rows use it; `plan-vs-scene.mjs` diffing
   419 pieces at 0.01 m; `built.mjs` serving the 69 from `dist/`.
7. **The budget, recorded, not asserted anew** (#834): the builder writes
   the lines `test/budget.mjs` prints before and after in `HISTORY.md`. The
   prototype's numbers are outer 1020, inner 702, outer plus outside 1151,
   inner plus outside 833, texture 43.7 MB.
8. **Idempotence**: a second `npm run assets:encode` prints "nothing to do"
   for all 69 and `git status` shows nothing new.

### Open calls

- **Provenance.** Recommend **a committed family made outside the repo, not
  the pipeline** (#830): the files are Blender 5.2's and raw, which #805's
  pin and #806's `finish.mjs` refuse, and making Devon rebuild 85 props in
  4.5 to satisfy a manifest is a day spent on something he did not ask for.
- **Copy Devon's scripts in.** Recommend **yes, into `tools/props/`, scripts
  and manifest only**: git is safer than one OneDrive folder, and without
  the `.blend` nothing there is an input file.
- **Size rule or family rule in the encoder.** Recommend **the size rule**
  (#831), because `CLAUDE.md` already states it, it covers the next small
  PNG from anywhere, and a family rule would be a second list to forget.
- **Swaps.** Recommend **none in this row**: a swap edits `mystery.json`
  and pins a box centre (#811), and doing one inside 70 dressing rows hides
  the one risky diff.
- **The three that look like clues.** Recommend **refuse them**: a player
  who sees a dagger or a body with no E prompt has been told something
  false.
- **The quay seven.** Recommend **rank 9**: nobody sees the quay from
  inside, so placing them now costs draws in both wards for nothing.
- **Lights.** Recommend **none** (#834): the props are dressing, and the
  light count is a grep `budget.mjs` fails on the day a second light
  source appears.
- **Negative `yOffset`.** Recommend **allow it** (#832): two rows need it,
  and the alternative is a surface-height field for two tabletops.
- **The chapel loft.** Recommend **`chapel-tower-2`** for the altar, the
  font and the pew. The chapel floor has three stations, five pressables
  and a flight, and an altar in it would sit 0.4 m from the apprentice's
  station.
- **Where the garden goes.** Recommend **the inner ward east of the
  Steward's chamber**, x 10.5 to 22, z 6.5 to 14. It is empty ground with
  no stop on it. Rank 6's garden comment is about a `garden` room id
  outside the east gate, and that ground stays unused.
- **The atlas's 1821 colours beside a 108-colour castle.** Recommend
  **accept, and ask the GPU look** (#53): nothing asserts a palette on a
  model, and the Kenney kit was never held to one either.
- **The editor.** Recommend **the id refusal and `base`** in
  `src/edit-mode.js`, since without the refusal the dev page stops
  loading on the next placed prop.

### Dependencies

- **Gate: none.** Needs `ktx` on PATH for the encode step only.
- **Lane B.** Not beside rank 1, 2a, 2b, 2e, 4, 9, or 13's increments 2
  and 3. **Before rank 1**, on Devon's request. Rank 1 finds `propPath`
  done and uses it, and its check 4 line adds `assets/blender` beside
  `assets/props`.
- **Rank 3**: the next GPU run is where the looking checklist below gets
  answered. Run it from a worktree.

### Constraints

- #390, #499, #506: each file and its row in one commit, 2.5 MB against
  200, every placed file encoded.
- #742, #831: the atlas stays a 128 px PNG, and check 9 holds it.
- #500, #832: one `propPath`, eight sites.
- #529: check 9 is `assets.mjs`'s and the check 1d line is `layout.mjs`'s.
  `plan-vs-scene.mjs` gains nothing.
- #584, #632: spliced, in the file's own ending.
- #611, #816, #834: no ceiling moves, and the numbers go in `HISTORY.md`.
- #785, #811, #814: nothing pressable moves, no station moves, and a row
  that fails a station moves itself.
- #36: no save change.
- #13, #34, #147: every line above has its break.
- #53: whether it reads as one castle is a GPU look.
- #830 to #834.

### Looking checklist

- [ ] The atlas beside the kit and the pixel stone: one castle, or a third
      look?
- [ ] The King's Hall from its door: the Constable's board, or a furniture
      shop?
- [ ] The cobwebs in their corners, the right way round and with holes.
