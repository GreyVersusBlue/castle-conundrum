...
(#624 to #630 on 2026-09-17, #708 to #715 on 2026-09-19, #729 to #736 on
...
the reload at Sext, the riddle, the third ring and the cook's walk (#729).
Six fixes to `test/play-castle.mjs` then carried the second run to the
accusation (#730 to #735), where the sentry and the porter went unread and
the ending came out wrong (#736). What is left is `src/interaction.js`'s
...
- **Nothing in `src/`.** The run is the deliverable. `test/play-castle.mjs` is
  ...
  2026-09-21 (#730 to #735): `shutPresent()` closes the Present list and the
...
- ~~The `snap('twelve-at-vespers')` beat~~ was routing straight across the
  inner ward into a wall; **fixed 2026-09-21** (#735), it now walks to the
...
- ~~`shots/play/` contains the numbered set, `twelve-at-vespers.png` among
  ...
  routing fix (#735) got the walked, numbered set past it on 2026-09-21.
...
- **Rank 1, the walker on the stair, shipped 2026-09-19 and is now confirmed
  on a GPU** (#716 to #720, #729, #731). `hike` no longer drives the player up
