...
Fifteen open rows: 1, 2a, 2b, 2c, 2d, 2e, 2f, 2g, 3, 4, 6, 7, 9, 11, 13. Rows are
...
| **B** | `data/scene-config.json`, and `test/tools.mjs`'s byte-exactness rail | 1, 2a, 2b, 2e, 2f, 2g, 4, 9, 13 (increments 2 and 3 only) |
| **C** | `data/npcs.json`'s `cast` block, and `npc.js`'s body machinery | 2c, 2d, 6 |
| **D** | `src/main.js`'s player rig and spawn | 6, 11 |
| **E** | `src/audio.js` and `data/sounds.json` | 2g (one `byRoom` line), 7 |
...
**Startable together right now:** 2f (lane B), then 2g (lanes B and E),
and after them rank 1 on
...
finds `propPath` already shipped. **2g, the chapel nave, follows 2f and
precedes rank 1** on the same terms: it is Devon's answer to 2f's pulpit and
rood, lane B, no Blender, no gate (#835). It also holds lane E for one line
of `data/sounds.json`, so it does not run beside rank 7 if rank 7 writes
that file. **Blender runs first, on Devon's own instruction** (#801). The order inside
