...
R2 sight at the body's height  -->  unlocked R3, 2026-09-21 (#775 to #777)
...
body's own height~~, filed 2026-09-21, shipped the same day (#775 to #777):
...
height is done (#775 to #777): rays at the feet plus `SIGHT_HEIGHTS`, a
...
| **R3** The GPU run | Opus 5 | **Local: GPU** | — | **Gate 1, above.** Its looking is done (#711 to #715); the old R1 walker holds on a GPU now (#734, #736), and a third sitting reached the accusation for the first time before a new bug, fixed sight heights in `src/interaction.js`, produced the wrong ending (#735 to #741). **Gated on R2, "Sight at the body's own height," which shipped 2026-09-21** (#775 to #777); the gate is open. **R1's own known break on this row is lifted on paper, not confirmed**: increment 2 gave `test/play-castle.mjs` the same `#start-mystery` door the other suites got (#771 to #774), and the file parses and the button and its wiring are in `dist/`, but nobody in a container can run `npm run play` to say the day actually carries through it (#53). Read R1's row, and treat that as this row's first act, not a settled fact. |
